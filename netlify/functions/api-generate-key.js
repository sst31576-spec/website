...
exports.handler = async function (event, context) {
    ...
    try {
        ...
        if (userStatus === 'Free') {
            const { rows: existingKeyRows } = await db.query('SELECT * FROM keys WHERE owner_discord_id = $1 AND key_type = $2 AND expires_at > NOW()', [id, 'temp']);
            ...
            if (response.data !== true) {
                console.error("Linkvertise Verification Failed:", response.data);
                return { statusCode: 403, body: JSON.stringify({ error: 'Linkvertise task not completed or already claimed.' }) };
            }
