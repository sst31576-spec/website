...
exports.handler = async function (event, context) {
    ...
    if (event.httpMethod === 'POST') {
        ...
        try {
            ...
            if (userPosition !== -1) { const lastReward = user.last_daily_reward_at ? new Date(user.last_daily_reward_at) : null; if (!lastReward || (now.getTime() - lastReward.getTime()) > 22 * 3600 * 1000) { let hoursToAdd = 0; if (userPosition === 0) hoursToAdd = 3; else if (userPosition === 1) hoursToAdd = 2; else if (userPosition === 2) hoursToAdd = 1; if (hoursToAdd > 0) { const { rows: keyRows } = await client.query('SELECT id, expires_at FROM keys WHERE owner_discord_id = $1 AND key_type = \'temp\' AND expires_at > NOW() ORDER BY expires_at DESC LIMIT 1', [id]); if (keyRows.length > 0) { const newExpiresAt = new Date(new Date(keyRows[0].expires_at).getTime() + hoursToAdd * 3600 * 1000); await client.query('UPDATE keys SET expires_at = $1 WHERE id = $2', [newExpiresAt, keyRows[0].id]); user.last_daily_reward_at = now.toISOString(); if (!responseData.notifications) responseData.notifications = []; responseData.notifications.push(`Congratulations! As a top player, you've been awarded ${hoursToAdd} free hour(s) on your key!`); } } } }
