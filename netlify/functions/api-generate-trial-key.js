...
exports.handler = async function (event, context) {
    ...
    try {
        ...
        const { rows: existingKeyRows } = await db.query('SELECT key_value FROM keys WHERE owner_discord_id = $1 AND expires_at > NOW()', [id]);
        ...
        return { statusCode: 200, body: JSON.stringify({ key: newKey, type: 'trial', expires: expiresAt }) };
    } catch (error) {
