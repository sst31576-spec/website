...
for (let i = 21; i <= 50; i++) { lastCps *= (5 + i * 0.1); lastCost *= (6 + i * 0.15); KING_GAME_UPGRADES[`b${i}`] = { name: highTierNames[i - 21] || `Cosmic Entity #${i - 20}`, baseCost: lastCost, costMultiplier: 1.23 + (i * 0.002), cps: lastCps };}
...
const getUpgradeCost = (u, c, a) => { const r = KING_GAME_UPGRADES[u]; let t = BigInt(Math.ceil(r.baseCost * Math.pow(r.costMultiplier, c))); return a && a.half_cost && new Date(a.half_cost) > new Date && (t /= BigInt(2)), t };
const getUnitCost = (u, c, a) => { const r = a ? TROOPS_CONFIG[u] : DEFENSES_CONFIG[u]; return BigInt(Math.ceil(r.cost * Math.pow(r.costMultiplier, c))) };
...
const calculateKingGameState = async (u, c) => { const a = u.king_game_upgrades || {}, r = u.active_boosts || {}, t = u => a[u] || 0; let e = BigInt(1) + BigInt(t("click") * (KING_GAME_UPGRADES.click?.value || 1)), o = BigInt(0); for (const i in KING_GAME_UPGRADES) "click" !== i && (o += BigInt(t(i)) * BigInt(Math.round(KING_GAME_UPGRADES[i].cps))); let s = Math.pow(2, u.prestige_level || 0), n = s; const l = RANKS_CONFIG.slice().reverse().find(c => u.power >= c.power) || RANKS_CONFIG[0]; n *= Math.pow(1.1, l.index); const g = c.findIndex(c => c.discord_id === u.discord_id); 0 === g && (n *= 1.5), 1 === g && (n *= 2); let p = e * BigInt(Math.round(n)), d = o * BigInt(Math.round(n)); return r.x2_coins && new Date(r.x2_coins) > new Date && (p *= BigInt(2), d *= BigInt(2)), { clickValue: p.toString(), cps: d.toString(), power: u.power.toString(), rank: l.name } };
...
exports.handler = async function (event, context) {
    ...
    if (event.httpMethod === 'POST') {
        ...
        try {
            ...
            // --- MODIFICATION / FIX ---
            // On recalcule l'état final AVANT de l'envoyer
            ...
            const finalResponse = {
                ...
                cps: finalGameState.cps,
                clickValue: finalGameState.clickValue,
                // FIN DE L'AJOUT
                ...responseData
            };
