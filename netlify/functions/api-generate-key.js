...
exports.handler = async function (event, context) {
    ...
    try {
        ...
        // Si utilisateur “Free”
        if (userStatus === 'Free') {
            // Vérifier s’il y a déjà une clé temporaire active
            ...
            const verificationUrl = `https://publisher.linkvertise.com/api/v1/anti_bypassing?token=${process.env.LINKVERTISE_API_TOKEN}&hash=${hash}`;
            ...
            const valid =
                resp === true ||
                resp === "TRUE" ||
                (typeof resp === 'object' && resp.success === true);

            if (!valid) {
                console.error("Linkvertise Verification Failed:", resp);
                return {
                    statusCode: 403,
                    body: JSON.stringify({ error: 'Linkvertise task not completed or already claimed.' })
                };
            }

            // Génération de la nouvelle clé temporaire
        ...
        if (userStatus === 'Perm') {
            ...
            return {
                statusCode: 200,
                body: JSON.stringify({ key: newKey, type: 'perm' })
            };
        }
