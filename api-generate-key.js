...
exports.handler = async function (event, context) {
    ...
    try {
        ...
        if (userStatus === 'Free') {
            ...
            const response = await axios.get(`https://publisher.linkvertise.com/api/v1/affiliate/link/check?token=${process.env.LINKVERTISE_API_TOKEN}&ip=${userIp}`);
            if (!response.data.success || !response.data.data.completed) return { statusCode: 403, body: JSON.stringify({ error: 'You must complete the Linkvertise task first.' }) };
        ...
        console.error('Key Generation Error:', error.response ? error.response.data : error.message);
        return { statusCode: 500, body: JSON.stringify({ error: 'Failed to generate key. Please complete the task and try again.' }) };
    }
};
