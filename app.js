...
function formatBigNumber(num) {
    ...
    let formattedValue;
    if (value >= 100) formattedValue = value.toFixed(1);
    else if (value >= 10) formattedValue = value.toFixed(2);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- NON-GAME PAGES FUNCTIONS (RESTORED) ---
    ...
    const renderLeaderboardModal = () => {
        ...
        const fetchAndDisplay = async (sortBy = 'power') => {
            ...
            try {
                ...
                if (players.length === 0) { container.innerHTML = '<p>No players to display.</p>'; return; }
                container.innerHTML = `<div class="leaderboard-table-container"><table class="leaderboard-table"><thead><tr><th>#</th><th>Player</th><th>Power</th><th>Coins</th><th>Prestige</th></tr></thead><tbody>${players.map((p, index) => `<tr><td>${index + 1}</td><td>${p.discord_username}</td><td>${formatBigNumber(p.power)}</td><td>${formatBigNumber(p.king_game_coins)}</td><td>${p.prestige_level}</td></tr>`).join('')}</tbody></table></div>`;
    ...
    const renderEarnTimePage = async () => { const container = document.getElementById('earn-time-content'); if (!container || !currentUser) return; container.innerHTML = '<p>Loading your empire...</p>'; try { await fetch('/api/earn-time'); renderKingGame(); } catch (error) { container.innerHTML = `<p class="error-message">Could not load game data. Only users with an active key can play.</p><a href="/get-key" class="discord-btn" style="margin-top: 15px;">Get a Key</a>`; } };
    
    const setupUserSearch = (inputId, dropdownId, onSelect, showPower) => {
        ...
        const updateDropdown = () => {
            ...
            const filteredUsers = allUsers.filter(u => u.discord_username.toLowerCase().startsWith(query) && u.discord_id !== currentUser.discord_id);
            dropdown.innerHTML = '';
            highlightedIndex = filteredUsers.length > 0 ? 0 : -1;
            if (filteredUsers.length > 0) {
                filteredUsers.slice(0, 5).forEach(user => {
                    const item = document.createElement('a');
                    item.innerHTML = `<span>${user.discord_username}</span>` + (showPower ? `<span class="power">⚡ ${formatBigNumber(user.power)}</span>` : "");
                    item.addEventListener("mousedown", () => {
                        searchInput.value = user.discord_username;
                        selectedUserId = user.discord_id;
                        dropdown.style.display = 'none';
                        onSelect(selectedUserId, searchInput);
                    });
