...
exports.handler = async function (event, context) {
    ...
    if (event.httpMethod === 'POST') {
        ...
        try {
            ...
            else if (action === 'attack_player') { const { targetId } = body; if (!targetId || targetId === id) throw new Error('Invalid target.'); const oldTop3 = topPlayers.map(p => p.discord_id); const lastAttacks = user.last_attack_timestamps || {}; if (lastAttacks[targetId] && (now.getTime() - new Date(lastAttacks[targetId]).getTime()) < 24 * 3600 * 1000) throw new Error('You can only attack this player once every 24 hours.'); if (topPlayers[1] && targetId === topPlayers[1].discord_id) throw new Error('The Queen is protected and cannot be attacked.'); const targetRes = await client.query('SELECT *, king_game_coins::text FROM users WHERE discord_id = $1 FOR UPDATE', [targetId]); if (targetRes.rows.length === 0) throw new Error('Target player not found.'); let targetUser = targetRes.rows[0]; targetUser.power = BigInt(targetUser.power || '0'); let attackerPower = user.power; let defenderPower = targetUser.power; const attackerPos = topPlayers.findIndex(p => p.discord_id === id); const defenderPos = topPlayers.findIndex(p => p.discord_id === targetId); if (attackerPos === 0) attackerPower = attackerPower * 2n; if (attackerPos === 2) attackerPower = attackerPower * 15n / 10n; if (defenderPos === 0) defenderPower = defenderPower * 2n; if (defenderPos === 2) defenderPower = defenderPower * 15n / 10n; const attackerRoll = Number(attackerPower) * (Math.random() * 0.5 + 0.75); const defenderRoll = Number(defenderPower) * (Math.random() * 0.5 + 0.75); const attackerWins = attackerRoll > defenderRoll; const calculateLosses = (unitData, lossPercent) => { if (!unitData) return {}; for (const unitId in unitData) { const losses = Math.ceil(unitData[unitId].quantity * lossPercent); unitData[unitId].quantity -= losses; if (unitData[unitId].quantity < 0) unitData[unitId].quantity = 0; } return unitData; }; if (attackerWins) { const stolenAmount = BigInt(targetUser.king_game_coins) / 10n; user.king_game_coins = (BigInt(user.king_game_coins) + stolenAmount).toString(); targetUser.king_game_coins = (BigInt(targetUser.king_game_coins) - stolenAmount).toString(); user.troops = calculateLosses(troops, 0.10); targetUser.troops = calculateLosses(targetUser.troops, 0.25); targetUser.defenses = calculateLosses(targetUser.defenses, 0.10); responseData.battleReport = `Victory! You stole ${stolenAmount.toLocaleString('en-US')} coins.`; } else { for(const troopId in troops) { if(!SPECIAL_UNITS_CONFIG[troopId]) troops[troopId].quantity = 0; } user.troops = troops; targetUser.troops = calculateLosses(targetUser.troops, 0.10); targetUser.defenses = calculateLosses(targetUser.defenses, 0.05); responseData.battleReport = `Total Defeat! Your regular army was wiped out.`; } user.power = calculatePower(user); targetUser.power = calculatePower(targetUser); await client.query('UPDATE users SET king_game_coins = $1, troops = $2, defenses = $3, power = $4 WHERE discord_id = $5', [targetUser.king_game_coins, JSON.stringify(targetUser.troops), JSON.stringify(targetUser.defenses), targetUser.power.toString(), targetId]); lastAttacks[targetId] = now.toISOString(); user.last_attack_timestamps = lastAttacks; const newTopPlayersRes = await client.query('SELECT discord_id, troops FROM users ORDER BY power DESC LIMIT 3'); const newTop3 = newTopPlayersRes.rows.map(p => p.discord_id); for (let i = 0; i < 3; i++) { const oldRulerId = oldTop3[i]; const newRulerId = newTop3[i]; if (oldRulerId && newRulerId && oldRulerId !== newRulerId) { const specialUnits = { 0: 'royal_guard', 1: 'queens_guard', 2: 'elite_soldier' }; const unitToTransfer = specialUnits[i]; const oldRulerRes = await client.query('SELECT troops FROM users WHERE discord_id = $1 FOR UPDATE', [oldRulerId]); let oldRulerTroops = oldRulerRes.rows[0].troops || {}; const newRulerRes = await client.query('SELECT troops FROM users WHERE discord_id = $1 FOR UPDATE', [newRulerId]); let newRulerTroops = newRulerRes.rows[0].troops || {}; const quantityToTransfer = oldRulerTroops[unitToTransfer]?.quantity || 0; if (quantityToTransfer > 0) { if (!newRulerTroops[unitToTransfer]) newRulerTroops[unitToTransfer] = { quantity: 0, level: 1 }; newRulerTroops[unitToTransfer].quantity += quantityToTransfer; oldRulerTroops[unitToTransfer].quantity = 0; await client.query('UPDATE users SET troops = $1 WHERE discord_id = $2', [JSON.stringify(oldRulerTroops), oldRulerId]); await client.query('UPDATE users SET troops = $1 WHERE discord_id = $2', [JSON.stringify(newRulerTroops), newRulerId]); } } } }
            ...
            const finalGameState = await calculateKingGameState(user, (await client.query('SELECT discord_id, power FROM users ORDER BY power DESC LIMIT 3')).rows);
            const finalResponse = {
