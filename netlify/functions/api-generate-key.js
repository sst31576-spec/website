...
exports.handler = async function (event, context) {
    ...
    try {
        ...
        // --- Logique pour les utilisateurs PERM ---
        if (userStatus === 'Perm') {
        ...
        if (userStatus === 'Free') {
            ...
            // 2. Si aucune clé, on exige la validation Linkvertise
            const body = event.body ? JSON.parse(event.body) : {};
            const { hash } = body;

            // Si le frontend n'envoie pas de hash, on lui dit d'afficher les boutons "Start Task"
            if (!hash) {
            ...
            // Logique de validation robuste pour accepter les différentes réponses de Linkvertise
